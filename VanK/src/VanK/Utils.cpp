...
namespace VanK
{
    ...
    std::string Utils::OpenFile(const char* filter)
    {
        ...
        SDL_ShowOpenFileDialog(callback, (void*)filters, window->getWindow(), filters, SDL_arraysize(filters), SDL_GetBasePath(), false);
    ...
    std::string Utils::SaveFile(const char* filter)
    {
        ...
        SDL_ShowSaveFileDialog(callback, nullptr, window->getWindow(), filters, SDL_arraysize(filters), nullptr);
