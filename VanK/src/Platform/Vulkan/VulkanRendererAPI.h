...
namespace VanK
{
    ...
    inline VkCommandBuffer Unwrap(VanKCommandBuffer cmd)
    {
    ...
    inline VkPipeline Unwrap(VanKPipeLine pipeline)
    {
        return pipeline ? pipeline->handle : VK_NULL_HANDLE;
    }

    inline VanKPipeLine Wrap(VkPipeline pipeline)
    {
        if (pipeline == VK_NULL_HANDLE)
            return nullptr;
        auto* wrapper = new VanKPipeLine_T;
        wrapper->handle = pipeline;
        return wrapper;
    }
...
#define VK_CHECK(vkFnc)                                                                                                \
  {                                                                                                                    \
    ...
    namespace utils
    {
        ...
        public:
            ...
            VkPhysicalDeviceSwapchainMaintenance1FeaturesEXT getSwapchainFeatures() const
            {
      ...
      raise(SIGTRAP);
            ...
            void initInstance()
            {
                ...
                // Allow debug utils (naming objects)
                ...
                if (extensionIsAvailable(VK_KHR_PORTABILITY_ENUMERATION_EXTENSION_NAME, m_instanceExtensionsAvailable))
            ...
            void getAvailableDeviceExtensions()
            {
                ...
                VK_CHECK(
                    vkEnumerateDeviceExtensionProperties(m_physicalDevice, nullptr, &count, m_deviceExtensionsAvailable.
                        data
                        ()));
            }
  ...
  bool m_enableValidationLayers = false;
        ...
        public:
            ...
            VkSemaphore getSignalSemaphores() const
            {
        ...
        public:
                                             ...
                                             VkBufferUsageFlags2KHR usageFlags)
            {
                ...
                vkCmdCopyBuffer(cmd, stagingBuffer.buffer, buffer.buffer, uint32_t(copyRegion.size()),
                                copyRegion.data());
        ...
        private:
            ...
            std::unordered_map<VkSamplerCreateInfo, VkSampler, SamplerCreateInfoHash, SamplerCreateInfoEqual>
            m_samplerMap
                {};
        ...
        public:
            ...
            void updateImGuiDescriptors(VkImageLayout layout = VK_IMAGE_LAYOUT_GENERAL)
            {
                if ((ImGui::GetCurrentContext() != nullptr) && ImGui::GetIO().BackendPlatformUserData != nullptr)
                {
                    for (size_t d = 0; d < m_res.descriptor.size(); ++d)
                    {
                        // Remove old descriptor if needed
                        if (m_descriptorSet[d])
                            ImGui_ImplVulkan_RemoveTexture(m_descriptorSet[d]);
                        m_descriptorSet[d] = ImGui_ImplVulkan_AddTexture(
                            m_createInfo.linearSampler, m_res.uiImageViews[d], layout);
                    }
                }
            }

            //--- Getters for the GBuffer resources -------------------------
            ImTextureID getImTextureID(uint32_t i = 0) const
            {
                return reinterpret_cast<ImTextureID>(m_descriptorSet[i]);
            }

            VkExtent2D getSize() const { return m_createInfo.size; }
            size_t getNumImGuiTextures() const { return m_createInfo.color.size(); }
            ...
            void create(VkCommandBuffer cmd)
            {
                ...
                for (uint32_t c = 0; c < numColor; c++)
                {
                    ...
                    }
                    {
                        ...
                        // Forcing the VIEW to have a 1 in the alpha channel
...

                {
                    ...
                    for (uint32_t c = 0; c < numColor; c++)
                    {
                        ...
                        vkCmdClearColorImage(cmd, m_res.gBufferColor[c].image, layout, &clear_value,
                                             uint32_t(range.size()),
                                             range.data());
                    }
                }
                ...
                if ((ImGui::GetCurrentContext() != nullptr) && ImGui::GetIO().BackendPlatformUserData != nullptr)
                {
                    ...
                    {
                        m_descriptorSet[d] = ImGui_ImplVulkan_AddTexture(
                            m_createInfo.linearSampler, m_res.uiImageViews[d],
                            layout);
                    }
                }
            }
    ...
    public:
        ...
        void SetWindowHandle(SDL_Window* window) override
        {
            m_window = window;
        }
        ...
        const VulkanShader* m_graphicsShader = nullptr;
        const VulkanShader* m_CircleShader = nullptr;
        ...
        void BindFragmentSamplers(VanKCommandBuffer cmd, uint32_t firstSlot, const TextureSamplerBinding* samplers,
                                  uint32_t num_bindings) override;

        VkSampler getSamplerPool(VkSamplerCreateInfo info) { return m_samplerPool.acquireSampler(info); }
    ...
    private:
        ...
        -*/
        void DispatchCompute(VanKComputePass* computePass, uint32_t groupCountX, uint32_t groupCountY,
                             uint32_t groupCountZ) override;
        
        void BindStorageBuffer(VanKCommandBuffer cmd, VanKPipelineBindPoint bindPoint, VanKBuffer* buffer, uint32_t set,
                               uint32_t binding, uint32_t
                               arrayElement = 0) override;
        void BindUniformBuffer(VanKCommandBuffer cmd, VanKPipelineBindPoint bindPoint, UniformBuffer* buffer,
                               uint32_t set,
                               uint32_t binding, uint32_t arrayElement) override;
        void BeginRendering(VanKCommandBuffer cmd, const VanKColorTargetInfo* color_target_info,
        ...
        void BindIndexBuffer(VanKCommandBuffer cmd, const IndexBuffer& indexBuffer,
                             VanKIndexElementSize elementSize) override;
        
        void BindPipeline(VanKCommandBuffer cmd, VanKPipelineBindPoint pipelineBindPoint,
                          VanKPipeLine pipeline) override;

        void DrawIndexed(VanKCommandBuffer cmd, uint32_t indexCount, uint32_t instanceCount, uint32_t firstIndex,
                         int32_t vertexOffset, uint32_t firstInstance) override;
        void Draw(VanKCommandBuffer cmd, uint32_t vertexCount, uint32_t instanceCount, uint32_t firstVertex,
    ...
    private:
        ...
        VkDescriptorSetLayout m_commonDescriptorSetLayout{}; // Descriptor set layout for the scene info (set 1)
        ...
        // Timeline semaphore used to synchronize CPU submission with GPU completion
        ...
        Uint8* m_downloadedEntityIDPtr = nullptr; // Pointer to mapped entity ID buffer
        utils::Buffer m_downloadedEntityIDBuffer{}; // Buffer handle for cleanup

        VkFormat ConvertToVkFormat(VanKTextureFormat format)
        {
            using enum VanKTextureFormat;
            switch (format)
            {
            case VanK_TEXTUREFORMAT_INVALID: return VK_FORMAT_UNDEFINED;

            // Unsigned Normalized Float Color Formats
            case VanK_TEXTUREFORMAT_A8_UNORM: return VK_FORMAT_R8_UNORM; // Approximate
            case VanK_TEXTUREFORMAT_R8_UNORM: return VK_FORMAT_R8_UNORM;
            case VanK_TEXTUREFORMAT_R8G8_UNORM: return VK_FORMAT_R8G8_UNORM;
            case VanK_TEXTUREFORMAT_R8G8B8A8_UNORM: return VK_FORMAT_R8G8B8A8_UNORM;
            case VanK_TEXTUREFORMAT_R16_UNORM: return VK_FORMAT_R16_UNORM;
            case VanK_TEXTUREFORMAT_R16G16_UNORM: return VK_FORMAT_R16G16_UNORM;
            case VanK_TEXTUREFORMAT_R16G16B16A16_UNORM: return VK_FORMAT_R16G16B16A16_UNORM;
            case VanK_TEXTUREFORMAT_R10G10B10A2_UNORM: return VK_FORMAT_A2B10G10R10_UNORM_PACK32;
            case VanK_TEXTUREFORMAT_B5G6R5_UNORM: return VK_FORMAT_B5G6R5_UNORM_PACK16;
            case VanK_TEXTUREFORMAT_B5G5R5A1_UNORM: return VK_FORMAT_B5G5R5A1_UNORM_PACK16;
            case VanK_TEXTUREFORMAT_B4G4R4A4_UNORM: return VK_FORMAT_B4G4R4A4_UNORM_PACK16;
            case VanK_TEXTUREFORMAT_B8G8R8A8_UNORM: return VK_FORMAT_B8G8R8A8_UNORM;

            // Compressed Unsigned Normalized Float Color Formats
            case VanK_TEXTUREFORMAT_BC1_RGBA_UNORM: return VK_FORMAT_BC1_RGBA_UNORM_BLOCK;
            case VanK_TEXTUREFORMAT_BC2_RGBA_UNORM: return VK_FORMAT_BC2_UNORM_BLOCK;
            case VanK_TEXTUREFORMAT_BC3_RGBA_UNORM: return VK_FORMAT_BC3_UNORM_BLOCK;
            case VanK_TEXTUREFORMAT_BC4_R_UNORM: return VK_FORMAT_BC4_UNORM_BLOCK;
            case VanK_TEXTUREFORMAT_BC5_RG_UNORM: return VK_FORMAT_BC5_UNORM_BLOCK;
            case VanK_TEXTUREFORMAT_BC7_RGBA_UNORM: return VK_FORMAT_BC7_UNORM_BLOCK;

            // Compressed Signed Float Color Formats
            case VanK_TEXTUREFORMAT_BC6H_RGB_FLOAT: return VK_FORMAT_BC6H_SFLOAT_BLOCK;

            // Compressed Unsigned Float Color Formats
            case VanK_TEXTUREFORMAT_BC6H_RGB_UFLOAT: return VK_FORMAT_BC6H_UFLOAT_BLOCK;

            // Signed Normalized Float Color Formats
            case VanK_TEXTUREFORMAT_R8_SNORM: return VK_FORMAT_R8_SNORM;
            case VanK_TEXTUREFORMAT_R8G8_SNORM: return VK_FORMAT_R8G8_SNORM;
            case VanK_TEXTUREFORMAT_R8G8B8A8_SNORM: return VK_FORMAT_R8G8B8A8_SNORM;
            case VanK_TEXTUREFORMAT_R16_SNORM: return VK_FORMAT_R16_SNORM;
            case VanK_TEXTUREFORMAT_R16G16_SNORM: return VK_FORMAT_R16G16_SNORM;
            case VanK_TEXTUREFORMAT_R16G16B16A16_SNORM: return VK_FORMAT_R16G16B16A16_SNORM;

            // Signed Float Color Formats
            case VanK_TEXTUREFORMAT_R16_FLOAT: return VK_FORMAT_R16_SFLOAT;
            case VanK_TEXTUREFORMAT_R16G16_FLOAT: return VK_FORMAT_R16G16_SFLOAT;
            case VanK_TEXTUREFORMAT_R16G16B16A16_FLOAT: return VK_FORMAT_R16G16B16A16_SFLOAT;
            case VanK_TEXTUREFORMAT_R32_FLOAT: return VK_FORMAT_R32_SFLOAT;
            case VanK_TEXTUREFORMAT_R32G32_FLOAT: return VK_FORMAT_R32G32_SFLOAT;
            case VanK_TEXTUREFORMAT_R32G32B32A32_FLOAT: return VK_FORMAT_R32G32B32A32_SFLOAT;

            // Unsigned Float Color Formats
            case VanK_TEXTUREFORMAT_R11G11B10_UFLOAT: return VK_FORMAT_B10G11R11_UFLOAT_PACK32;

            // Unsigned Integer Color Formats
            case VanK_TEXTUREFORMAT_R8_UINT: return VK_FORMAT_R8_UINT;
            case VanK_TEXTUREFORMAT_R8G8_UINT: return VK_FORMAT_R8G8_UINT;
            case VanK_TEXTUREFORMAT_R8G8B8A8_UINT: return VK_FORMAT_R8G8B8A8_UINT;
            case VanK_TEXTUREFORMAT_R16_UINT: return VK_FORMAT_R16_UINT;
            case VanK_TEXTUREFORMAT_R16G16_UINT: return VK_FORMAT_R16G16_UINT;
            case VanK_TEXTUREFORMAT_R16G16B16A16_UINT: return VK_FORMAT_R16G16B16A16_UINT;
            case VanK_TEXTUREFORMAT_R32_UINT: return VK_FORMAT_R32_UINT;
            case VanK_TEXTUREFORMAT_R32G32_UINT: return VK_FORMAT_R32G32_UINT;
            case VanK_TEXTUREFORMAT_R32G32B32A32_UINT: return VK_FORMAT_R32G32B32A32_UINT;

            // Signed Integer Color Formats
            case VanK_TEXTUREFORMAT_R8_INT: return VK_FORMAT_R8_SINT;
            case VanK_TEXTUREFORMAT_R8G8_INT: return VK_FORMAT_R8G8_SINT;
            case VanK_TEXTUREFORMAT_R8G8B8A8_INT: return VK_FORMAT_R8G8B8A8_SINT;
            case VanK_TEXTUREFORMAT_R16_INT: return VK_FORMAT_R16_SINT;
            case VanK_TEXTUREFORMAT_R16G16_INT: return VK_FORMAT_R16G16_SINT;
            case VanK_TEXTUREFORMAT_R16G16B16A16_INT: return VK_FORMAT_R16G16B16A16_SINT;
            case VanK_TEXTUREFORMAT_R32_INT: return VK_FORMAT_R32_SINT;
            case VanK_TEXTUREFORMAT_R32G32_INT: return VK_FORMAT_R32G32_SINT;
            case VanK_TEXTUREFORMAT_R32G32B32A32_INT: return VK_FORMAT_R32G32B32A32_SINT;

            // SRGB Unsigned Normalized Color Formats
            case VanK_TEXTUREFORMAT_R8G8B8A8_UNORM_SRGB: return VK_FORMAT_R8G8B8A8_SRGB;
            case VanK_TEXTUREFORMAT_B8G8R8A8_UNORM_SRGB: return VK_FORMAT_B8G8R8A8_SRGB;

            // Compressed SRGB Unsigned Normalized Color Formats
            case VanK_TEXTUREFORMAT_BC1_RGBA_UNORM_SRGB: return VK_FORMAT_BC1_RGBA_SRGB_BLOCK;
            case VanK_TEXTUREFORMAT_BC2_RGBA_UNORM_SRGB: return VK_FORMAT_BC2_SRGB_BLOCK;
            case VanK_TEXTUREFORMAT_BC3_RGBA_UNORM_SRGB: return VK_FORMAT_BC3_SRGB_BLOCK;
            case VanK_TEXTUREFORMAT_BC7_RGBA_UNORM_SRGB: return VK_FORMAT_BC7_SRGB_BLOCK;

            // Depth Formats
            case VanK_TEXTUREFORMAT_D16_UNORM: return VK_FORMAT_D16_UNORM;
            case VanK_TEXTUREFORMAT_D24_UNORM: return VK_FORMAT_X8_D24_UNORM_PACK32;
            case VanK_TEXTUREFORMAT_D32_FLOAT: return VK_FORMAT_D32_SFLOAT;
            case VanK_TEXTUREFORMAT_D24_UNORM_S8_UINT: return VK_FORMAT_D24_UNORM_S8_UINT;
            case VanK_TEXTUREFORMAT_D32_FLOAT_S8_UINT: return VK_FORMAT_D32_SFLOAT_S8_UINT;

            // ASTC (compressed UNORM)
            case VanK_TEXTUREFORMAT_ASTC_4x4_UNORM: return VK_FORMAT_ASTC_4x4_UNORM_BLOCK;
            case VanK_TEXTUREFORMAT_ASTC_5x4_UNORM: return VK_FORMAT_ASTC_5x4_UNORM_BLOCK;
            case VanK_TEXTUREFORMAT_ASTC_5x5_UNORM: return VK_FORMAT_ASTC_5x5_UNORM_BLOCK;
            case VanK_TEXTUREFORMAT_ASTC_6x5_UNORM: return VK_FORMAT_ASTC_6x5_UNORM_BLOCK;
            case VanK_TEXTUREFORMAT_ASTC_6x6_UNORM: return VK_FORMAT_ASTC_6x6_UNORM_BLOCK;
            case VanK_TEXTUREFORMAT_ASTC_8x5_UNORM: return VK_FORMAT_ASTC_8x5_UNORM_BLOCK;
            case VanK_TEXTUREFORMAT_ASTC_8x6_UNORM: return VK_FORMAT_ASTC_8x6_UNORM_BLOCK;
            case VanK_TEXTUREFORMAT_ASTC_8x8_UNORM: return VK_FORMAT_ASTC_8x8_UNORM_BLOCK;
            case VanK_TEXTUREFORMAT_ASTC_10x5_UNORM: return VK_FORMAT_ASTC_10x5_UNORM_BLOCK;
            case VanK_TEXTUREFORMAT_ASTC_10x6_UNORM: return VK_FORMAT_ASTC_10x6_UNORM_BLOCK;
            case VanK_TEXTUREFORMAT_ASTC_10x8_UNORM: return VK_FORMAT_ASTC_10x8_UNORM_BLOCK;
            case VanK_TEXTUREFORMAT_ASTC_10x10_UNORM: return VK_FORMAT_ASTC_10x10_UNORM_BLOCK;
            case VanK_TEXTUREFORMAT_ASTC_12x10_UNORM: return VK_FORMAT_ASTC_12x10_UNORM_BLOCK;
            case VanK_TEXTUREFORMAT_ASTC_12x12_UNORM: return VK_FORMAT_ASTC_12x12_UNORM_BLOCK;

            // ASTC (compressed SRGB)
            case VanK_TEXTUREFORMAT_ASTC_4x4_UNORM_SRGB: return VK_FORMAT_ASTC_4x4_SRGB_BLOCK;
            case VanK_TEXTUREFORMAT_ASTC_5x4_UNORM_SRGB: return VK_FORMAT_ASTC_5x4_SRGB_BLOCK;
            case VanK_TEXTUREFORMAT_ASTC_5x5_UNORM_SRGB: return VK_FORMAT_ASTC_5x5_SRGB_BLOCK;
            case VanK_TEXTUREFORMAT_ASTC_6x5_UNORM_SRGB: return VK_FORMAT_ASTC_6x5_SRGB_BLOCK;
            case VanK_TEXTUREFORMAT_ASTC_6x6_UNORM_SRGB: return VK_FORMAT_ASTC_6x6_SRGB_BLOCK;
            case VanK_TEXTUREFORMAT_ASTC_8x5_UNORM_SRGB: return VK_FORMAT_ASTC_8x5_SRGB_BLOCK;
            case VanK_TEXTUREFORMAT_ASTC_8x6_UNORM_SRGB: return VK_FORMAT_ASTC_8x6_SRGB_BLOCK;
            case VanK_TEXTUREFORMAT_ASTC_8x8_UNORM_SRGB: return VK_FORMAT_ASTC_8x8_SRGB_BLOCK;
            case VanK_TEXTUREFORMAT_ASTC_10x5_UNORM_SRGB: return VK_FORMAT_ASTC_10x5_SRGB_BLOCK;
            case VanK_TEXTUREFORMAT_ASTC_10x6_UNORM_SRGB: return VK_FORMAT_ASTC_10x6_SRGB_BLOCK;
            case VanK_TEXTUREFORMAT_ASTC_10x8_UNORM_SRGB: return VK_FORMAT_ASTC_10x8_SRGB_BLOCK;
            case VanK_TEXTUREFORMAT_ASTC_10x10_UNORM_SRGB: return VK_FORMAT_ASTC_10x10_SRGB_BLOCK;
            case VanK_TEXTUREFORMAT_ASTC_12x10_UNORM_SRGB: return VK_FORMAT_ASTC_12x10_SRGB_BLOCK;
            case VanK_TEXTUREFORMAT_ASTC_12x12_UNORM_SRGB: return VK_FORMAT_ASTC_12x12_SRGB_BLOCK;

            // ASTC Float formats - No Vulkan equivalents
            default: return VK_FORMAT_UNDEFINED;
            }
        }

        VkAttachmentLoadOp ConvertToVkLoadOp(VanKLoadOp op)
        {
            switch (op)
            {
            case VanK_LOADOP_LOAD: return VK_ATTACHMENT_LOAD_OP_LOAD;
            case VanK_LOADOP_CLEAR: return VK_ATTACHMENT_LOAD_OP_CLEAR;
            case VanK_LOADOP_DONT_CARE: return VK_ATTACHMENT_LOAD_OP_DONT_CARE;
            default: return VK_ATTACHMENT_LOAD_OP_DONT_CARE; // safe fallback
            }
        }

        VkAttachmentStoreOp ConvertToVkStoreOp(VanKStoreOp op)
        {
            switch (op)
            {
        ...
        VkClearColorValue ConvertToVkClearColor(const VanK_FColor& color, VkFormat format)
        {
        ...
        VkClearDepthStencilValue ConvertToVkClearDepthStencil(const VanK_FColor& color)
        {
            return VkClearDepthStencilValue{
                .depth = color.f[0], // Depth in [0, 1]
                .stencil = static_cast<uint32_t>(color.f[1]) // Stencil
            };
        }

        VkShaderStageFlags ConvertToVkShaderStageFlagBits(VanKShaderStageFlags flags)
        {
            switch (flags)
            {
            case VanKGraphics: return VK_SHADER_STAGE_ALL_GRAPHICS;
            case VanKCompute: return VK_SHADER_STAGE_COMPUTE_BIT;
            }
            return VK_SHADER_STAGE_ALL;
        }

        VkPrimitiveTopology ConvertToVkPrimitiveTopology(VankPrimitiveToplogy toplogy)
        {
            switch (toplogy)
            {
            case VanK_PRIMITIVE_TOPOLOGY_POINT_LIST: return VK_PRIMITIVE_TOPOLOGY_POINT_LIST;
            case VanK_PRIMITIVE_TOPOLOGY_LINE_LIST: return VK_PRIMITIVE_TOPOLOGY_LINE_LIST;
            case VanK_PRIMITIVE_TOPOLOGY_LINE_STRIP: return VK_PRIMITIVE_TOPOLOGY_LINE_STRIP;
            case VanK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST: return VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST;
            case VanK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP: return VK_PRIMITIVE_TOPOLOGY_TRIANGLE_STRIP;
            case VanK_PRIMITIVE_TOPOLOGY_TRIANGLE_FAN: return VK_PRIMITIVE_TOPOLOGY_TRIANGLE_FAN;
            ...
            case VanK_PRIMITIVE_TOPOLOGY_PATCH_LIST: return VK_PRIMITIVE_TOPOLOGY_PATCH_LIST;
            case VanK_PRIMITIVE_TOPOLOGY_MAX_ENUM: return VK_PRIMITIVE_TOPOLOGY_MAX_ENUM;
            }

            return VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST;
        }

        VkPolygonMode ConvertToVkPolygonMode(VanKPolygonMode polygonMode)
        {
            switch (polygonMode)
            {
            case VanK_POLYGON_MODE_FILL: return VK_POLYGON_MODE_FILL;
            case VanK_POLYGON_MODE_LINE: return VK_POLYGON_MODE_LINE;
            case VanK_POLYGON_MODE_POINT: return VK_POLYGON_MODE_POINT;
            case VanK_POLYGON_MODE_FILL_RECTANGLE_NV: return VK_POLYGON_MODE_FILL_RECTANGLE_NV;
            case VanK_POLYGON_MODE_MAX_ENUM: return VK_POLYGON_MODE_MAX_ENUM;
            }
            return VK_POLYGON_MODE_FILL;
        }

        VkCullModeFlags ConvertToVkCullMode(VanKCullModeFlags cullMode)
        {
            switch (cullMode)
            {
            case VanK_CULL_MODE_NONE: return VK_CULL_MODE_NONE;
            case VanK_CULL_MODE_FRONT_BIT: return VK_CULL_MODE_FRONT_BIT;
            case VanK_CULL_MODE_BACK_BIT: return VK_CULL_MODE_BACK_BIT;
            case VanK_CULL_MODE_FRONT_AND_BACK: return VK_CULL_MODE_FRONT_AND_BACK;
            case VanK_CULL_MODE_FLAG_BITS_MAX_ENUM: return VK_CULL_MODE_FLAG_BITS_MAX_ENUM;
            }
            return VK_CULL_MODE_NONE;
        }

        VkFrontFace ConvertToVkFrontFace(VanKFrontFace frontFace)
        {
            switch (frontFace)
            {
            case VanK_FRONT_FACE_COUNTER_CLOCKWISE: return VK_FRONT_FACE_CLOCKWISE;
            case VanK_FRONT_FACE_CLOCKWISE: return VK_FRONT_FACE_CLOCKWISE;
            case VanK_FRONT_FACE_MAX_ENUM: return VK_FRONT_FACE_MAX_ENUM;
            }
